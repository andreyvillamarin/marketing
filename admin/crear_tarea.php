...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nombre_tarea) || empty($fecha_vencimiento) || empty($prioridad) || empty($miembros_asignados)) {
        ...
        try {
            ...
            $stmt_miembros_email = $pdo->prepare("SELECT email, nombre_completo FROM usuarios WHERE id_usuario = ?");
            foreach ($miembros_asignados as $id_miembro) {
                $stmt_miembros_email->execute([$id_miembro]);
                $miembro = $stmt_miembros_email->fetch();
                if ($miembro) {
                    $asunto = "Nueva Tarea Asignada: " . $nombre_tarea;
                    $cuerpo = "<h1>Nueva Tarea</h1><p>Hola ".e($miembro['nombre_completo']).",</p><p>Se te ha asignado una nueva tarea: <strong>".e($nombre_tarea)."</strong> con fecha de vencimiento para el ".date('d/m/Y', strtotime($fecha_vencimiento)).".</p><p>Puedes ver los detalles ingresando a la plataforma.</p>";
                    enviar_email($miembro['email'], $miembro['nombre_completo'], $asunto, $cuerpo);
                }
            }
            if ($_SESSION['user_rol'] === 'analista') {
